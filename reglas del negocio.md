...
- El conteo físico se guarda aparte de `productos.stock` (autosave por fila, sin botón "Guardar" general). La diferencia (contado − stock del sistema al momento de contar) **no toca el stock real** hasta que se aplica esa fila puntual.
- **El ajuste de stock se aplica producto por producto** (botón ✓ en cada fila, `toma_aplicar_producto`), no todo junto al cerrar la sesión — por delta (`stock = stock + (contado − stock_sistema_al_contar)`), nunca sobrescribiendo el valor absoluto, así no se pierden ventas ocurridas mientras duró el conteo. Una fila ya aplicada no se puede volver a editar ni reaplicar.
